...
OpenLayers.Control.NLibNavigation = OpenLayers.Class(OpenLayers.Control, {
    ...
    initialize: function(options) {
        this.position = new OpenLayers.Pixel(5, 10);
    ...
    draw: function(px) {
        var img = "url(" + OpenLayers.Util.getImageLocation("mapcontrols3.png") + ")";
        var bimg = "url(" + OpenLayers.Util.getImageLocation("blank.gif") + ")";
        OpenLayers.Control.prototype.draw.apply(this, arguments);
        px = this.position || px;
        setPos(this.div, px.x, px.y, 64.5, 190, 1100, this);
        var self = this;
    ...
    zoomend: function() {
        ...
        bar2.style.top = (pos) + "px";
        pos += toint(bar2.style.height);
        bar3.style.top = (pos) + "px";
        bar3.style.height = (toint(bar.style.height) - pos + toint(bar2.style.height)) + "px";
    },
    onclick: function(e) {
        e = e || window.event;
        stopEvent(e);
    ...
    CLASS_NAME: "OpenLayers.Control.NLibNavigation"
});
    ...
    for (var p = 0; p < points.length; p++) {
        ...
        for (var i = 0; i < zu.length; i++) {
            ...
            if (dian.length == 1) {
                ...
                var imgUrl = points[p].IMG == null ? "/gsm/resource/js/map/img/marker_red.png" : points[p].IMG;
...
function drawmapshowperson(points, shouPopup) {
    ...
    var lonlat = new OpenLayers.LonLat(x, y);
    var imgUrl = points.IMG == null ? "/gsm/resource/js/map/img/marker_red.png" : points.IMG;
    var icon = new OpenLayers.Icon(imgUrl);
    var feature = new OpenLayers.Feature(markers, lonlat, { 'icon': icon });
    feature.popupClass = OpenLayers.Popup.FramedCloud;
...
function startTrack(readerCoo) {
    if (readerCoo == null || readerCoo.length <= 1) {
        ...
        var imgUrl = "/gsm/resource/js/map/img/guiji_ren.gif";
...
function fn(readerCoo, vectorLayer, markers, lineFeature, x) {
    if (x < readerCoo.length) {
        ...
        markers.clearMarkers();
        var size_ren = new OpenLayers.Size(27, 42);
        var imgUrlren = "/gsm/resource/js/map/img/guiji_ren.gif";
...
function initialGMaps(para) {
    ...
    var layeryector = new OpenLayers.Layer.DYCache("layeryector", 'http://192.168.2.20:3795/map/layerbase',
    {
        resolutions: [
                   0.00475892201166056,
                   0.00237946100583028,
        ...
        "default": new OpenLayers.Style(null, {
            rules: [
                new OpenLayers.Rule({
                    symbolizer: {
                        "Point": {
                            externalGraphic: "/gsm/resource/js/map/img/marker_red.png",
                            graphicXOffset: -5,
                            graphicYOffset: -16,
                            graphicWidth: 15,
                            graphicHeight: 17
        ...
        "temporary": new OpenLayers.Style(null, {
            rules: [
                new OpenLayers.Rule({
                    ...
                    tile: {
                        "Image": { externalGraphic: "/gsm/resource/js/map/img/marker.png" }
                    }
                })
            ]
        })
    ...
    if (para.points.length > 0) {
        ...
        //中心点
        var centerpoint = new NMap.LonLat(118.170211, 39.652227);

...
var addFun=function(){
	bootbox.dialog({
		 url:"${base}/zongzhitraining/materialmanagement/addLoad",
		  title: "材料管理",
...
function editFun(code){
	if(code&&""!=$.trim(code)){
		var dialog=bootbox.dialog({
			 url:"${base}/zongzhitraining/materialmanagement/modifyLoad?code="+code,
			  title: "材料管理",
			  style:'width:800px;height:550px;'
			});
	}else{
...
var deleteFun=function(code){
	 ...
	 bootbox.confirm('确定删除吗？',function(result){
		if(result){
			 $.ajax({
					type : "post",
					url : "${base}/zongzhitraining/materialmanagement/delete",
					data:{"codes":code},
					dataType : 'json',
					async : false,
...
function downloadCountAdd(code,downloadCount){
	...
	var data = "code="+code+"&downloadCount="+downloadCount;
	 $.ajax({
			type : "post",
			url : "${base}/zongzhitraining/materialmanagement/modify?t="+Math.random(),
			data : data,
			dataType : 'json',
			async : false,
			success : function(data) {
...
var seeFile = function(downloadCount,code,materialName,girdCode){
    	bootbox.dialog({
				 url:"${base}/zongzhitraining/materialmanagement/infoSee?code="+code,
	...
	$(function(){
		...
		layer.use('extend/layer.ext.js', function(){
		 ...
		 $("#grid-table").jqGrid({
			 	url:"${base }/zongzhitraining/materialmanagement/queryManage?remark=0",
		            ...
		            {name:'caozuo',width:95,fixed:true, sortable:false, resize:false,formatter:
		            	function(cellvalue, options, rowObject){
			            	...
			            	div+="<div class='ui-pg-div ui-inline-edit'title='编辑' style='float:left;cursor:pointer;' onclick='editFun(\""+code+"\")'>";
				            	...
				            	if(codeMaterialType == 0){
				            		div+="<div id class='ui-pg-div ui-inline-del'title='查看' style='float:left;margin-left:5px;'onclick='seeFile(\""+downloadCount+"\",\""+code+"\",\""+materialName+"\",\""+girdCode+"\")'>";
		            				div+='<span id class="ui-icon icon-zoom-in  "></span>';
				            	}else if(codeMaterialType == 1){
				            		div+="<div id class='ui-pg-div ui-inline-del'title='下载' style='float:left;margin-left:5px;'onclick='downloadfile(\""+fCode+"\",\""+downloadCount+"\",\""+code+"\",\""+girdCode+"\")'>";
		            				div+='<span id class="ui-icon icon-cloud-download  "></span>';
				            	}			
	           					div+='</div>';
	           					div+='</div>';

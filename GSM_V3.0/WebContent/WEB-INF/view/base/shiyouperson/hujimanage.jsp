...
<html lang="en">
	...
	$(function(){
		 $("#grid-table").jqGrid({
			 	url : "${base }/base/perhuji/queryManage",
		        defaultCaozuo:false,
		        ondblClickRow:function(rowid,iRow,iCol,e){
			    	  var code=$('#grid-table').jqGrid('getGridParam', "selrow");
			    	  var rowData = $('#grid-table').jqGrid("getRowData",code);
			    	  bootbox.dialog({
							 url:"${base}/base/person/infoSee?personCode="+rowData.personCode+"&personType=huji",
							  title: "人口信息采集表",
							  style:'width:1000px;height:600px;',
							  showFunctionBtn:false, 
							});
			      },
		        colNames:[ '', '','所属辖区','楼栋','单元','房屋','身份证号','姓名','性别','人户一致标识','人户状态','与户主关系'],
		        colModel:[
					...
					{name:'isRenHuYiZhi',index:'isRenHuYiZhi',sortable:true, width:'10%',formatter:function(cellvalue){
						if(cellvalue==true){
							return "是";
						}
							return "否";
					}},
					{name:'person.dicValueMap.codeHouseholdStatus',sortable:false, width:'10%'},
		            {name:'dicValueMap.codeOwnerRelation',sortable:false, width:'15%'}
										...
										<tr>
											<td >
												<div class="search-group">
													...
													<label class="col-xs-2 col-myxs-15 control-label text-right">人户一致标识</label>
													<select name="isRenHuYiZhi"  class=" myselect col-xs-2 ">
														<option value="">请选择</option>
														<option value="1">是</option>
														<option value="0">否</option>
													</select>
													<label class="col-xs-2 col-myxs-15 control-label text-right" >管理类型</label>
													<input type="text"  name="codeManageType" class="col-xs-2 " />
													<label class="col-xs-2 col-myxs-15 control-label text-right">与户主关系 </label>
													<input type="text" max-length="50"id="codeOwnerRelation" name="codeOwnerRelation" class="col-xs-2 " />

...
$(function(){
	...
	if('${vdata.difficultparty.sex }' == '男'){
		 $("#sex option[value='男']").attr("selected", "selected"); 
	 }else if('${vdata.difficultparty.sex }' == '女'){
		 $("#sex option[value='女']").attr("selected", "selected"); 
	 }else if('${vdata.difficultparty.sex }' == 0){
		 $("#sex option[value='0']").attr("selected", "selected"); 
	 }
	
	if('${vdata.difficultparty.codePartyType}'!=""){
...
function getValue(personCode,name,sex,cid,codeNation,codeEducation,codePolitical){
	if(personCode!=""){
...
<div class="row">
		<div class="col-xs-12" id="body">
			...
			<input type="hidden" name="code" value="${vdata.difficultparty.code}" />
			<input type="hidden" name="personCode" id="personCode" value="${vdata.difficultparty.personCode}" />
			<input type="hidden" name="position" id="position" class="noshow" value="${vdata.difficultparty.position}" />
			<input type="hidden" id="codeNation" name="codeNation" value="${vdata.difficultparty.codeNation}" />
			<input type="hidden" id="codeEducation" name="codeEducation" value="${vdata.difficultparty.codeEducation }"/>
			<input type="hidden" id="codePolitical" name="codePolitical" value="${vdata.difficultparty.codePolitical}"/>
			<input type="hidden" id="codePartyType" name="codePartyType" value="${vdata.difficultparty.codePartyType}"/>
			...
			<input type="hidden" id="zyzparty1" />
				<div class="form-group">
					<label id="sss" class="col-xs-2 control-label no-padding-right text-right" for="gridCode">所属辖区</label>
			
					<div class="col-xs-10">
						<input type="text"id="gridCode" name="gridCode" value="${vdata.difficultparty.gridCode }" class="col-xs-12 col-sm-12" />					
<%-- 						<input type="text" max-length="50"id="gridCode" name="gridCode" value="${vdata.difficultparty.gridCode}" class="col-xs-12 col-sm-12 tooltip-error required" /> --%>
					</div>
				...
				<div class="form-group">
					...
					<div class="col-xs-4">
						<div class="clearfix">
						<input type="text" max-length="50"id="name" name="name" value="${vdata.difficultparty.name}" placeholder="请双击输入框" ondblclick="openPerssonInfo()" class="col-xs-12 col-sm-12 tooltip-error required" />
						</div>
					</div>
				...
				<div class="form-group">	
					...
					<label class="col-xs-2 control-label no-padding-right text-right" for="cid">身份证 </label>
			
					<div class="col-xs-10">
						<div class="clearfix">
						<input type="text" max-length="50"id="cid" name="cid" value="${vdata.difficultparty.cid}" class="col-xs-12 col-sm-12 tooltip-error required" />
						</div>
					</div>
				</div>
				<div class="form-group">
					...
					<div class="col-xs-4">
						<select  id="codePartyType" name = "codePartyType"  class="form-control myselect"  style="width: 100%;" onchange="changeSelect(this.value)">
                           ...
                           <option value="kunnanparty" selected="true">困难党员</option>
					...
					<div class="col-xs-10 input-group">
						<input class="form-control date-picker tooltip-error required" name="inPartyDate" value="${vdata.difficultparty.inPartyDate}"  type="text">
						<span class="input-group-addon">
							<i class="icon-calendar bigger-110"></i>
						</span>
					</div>
<!-- 					<label class="col-xs-2 control-label no-padding-right text-right" for="inPartyDate">入党时间</label> -->
					
<!-- 					<div class="col-xs-4"> -->
<!-- 						<div class="clearfix"> -->
<%-- 						<input type="text" max-length="50"id="inPartyDate" name="inPartyDate" value="${vdata.difficultparty.inPartyDate}" class="col-xs-12 col-sm-12 tooltip-error required" /> --%>
<!-- 						</div> -->
<!-- 					</div> -->
				</div>
				<div class="form-group">
				...
				<div class="form-group">
					...
					<div class="col-xs-10">
						<div class="clearfix">
								<textarea id="remark" name="remark" class="col-xs-12 col-sm-12">${vdata.difficultparty.remark }</textarea>
						</div>
					</div>
				</div>

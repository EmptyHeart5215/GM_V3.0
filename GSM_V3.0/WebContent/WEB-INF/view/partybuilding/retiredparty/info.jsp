...
$(function(){
	...
	if('${vdata.retiredparty.sex }' == '男'){
		 $("#sex option[value='男']").attr("selected", "selected"); 
	 }else if('${vdata.retiredparty.sex }' == '女'){
		 $("#sex option[value='女']").attr("selected", "selected"); 
	 }else if('${vdata.retiredparty.sex }' == 0){
		 $("#sex option[value='0']").attr("selected", "selected"); 
	 }
	
	 if('${vdata.retiredparty.codePartyType}'!=""){
...
<div class="row">
		<div class="col-xs-12" id="body">
			...
			<input type="hidden" name="personCode" id="personCode" value="${vdata.retiredparty.personCode}" />
			<input type="hidden" name="position" id="position" class="noshow" value="${vdata.retiredparty.position}" />
			<input type="hidden" id="codeNation" name="codeNation" value="${vdata.retiredparty.codeNation}" />
			<input type="hidden" id="codeEducation" name="codeEducation" value="${vdata.retiredparty.codeEducation }"/>
			<input type="hidden" id="codePolitical" name="codePolitical" value="${vdata.retiredparty.codePolitical}"/>
			<input type="hidden" id="codePartyType" name="codePartyType" value="${vdata.retiredparty.codePartyType}"/>
			...
			<input type="hidden" id="zyzparty1" />
				<div class="form-group">
					...
					<div class="col-xs-10">
					<input type="text"id="gridCode" name="gridCode" value="${vdata.retiredparty.gridCode }" class="col-xs-12 col-sm-12" />
<%-- 						<input type="text" max-length="50"id="gridCode" name="gridCode" value="${vdata.retiredparty.gridCode}" class="col-xs-12 col-sm-12 tooltip-error required" /> --%>
				...
				<div class="form-group">
					...
					<div class="col-xs-4">
						<div class="clearfix">
						<input type="text" max-length="50"id="name" name="name" value="${vdata.retiredparty.name}" placeholder="请双击输入框" ondblclick="openPerssonInfo()" class="col-xs-12 col-sm-12 tooltip-error required" />
						</div>
					</div>
				...
				<div class="form-group">
					...
					<div class="col-xs-4">
						<select  id="codePartyType" name = "codePartyType"  class="form-control myselect"  style="width: 100%;" onchange="changeSelect(this.value)">
                           ...
                           <option value="tuixiuparty" selected="true">退休党员</option>
					...
					<div class="col-xs-10 input-group">
						<input class="form-control date-picker tooltip-error required" name="inPartyDate" value="${vdata.retiredparty.inPartyDate}"  type="text">
...
<!-- 					</div> -->
				...
				<div class="form-group">
					...
					<div class="col-xs-10">
						<div class="clearfix">
								<textarea id="remark" name="remark" class="col-xs-12 col-sm-12">${vdata.retiredparty.remark }</textarea>
						</div>
					</div>
				</div>

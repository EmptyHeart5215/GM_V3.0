...
<html lang="en">
	<head>
		...
		var addFun=function(){
			var dialog=bootbox.dialog({
				 url:"${base}/partybuilding/straightparty/addLoad",
				  title: "直辖党员信息",
		...
		function editFun(code){
			if(code&&""!=$.trim(code)){
				var dialog=bootbox.dialog({
				 url:"${base}/partybuilding/straightparty/modifyLoad?code="+code,
				  title: "直辖党员信息",
				  iframeStyle:"height:400px;width:730px;",
				  style:'width:800px;height:520px;'
				});
		...
		var deleteFun=function(code){
			...
			bootbox.confirm('确定删除吗？',function(result){
				if(result){
					 $.ajax({
							type : "post",
							url : "${base}/partybuilding/straightparty/delete?codes="+code,			
							dataType : 'json',
							async : false,
							success : function(data) {
	...
	$(function(){
		 $("#grid-table").jqGrid({
			 	url : "${base }/partybuilding/straightparty/queryManage",
			 	ondblClickRow:function(code){
				    bootbox.dialog({
						url:"${base }/partybuilding/straightparty/infoSee?code="+code,
						title: "详细信息表",

...
var addFun=function(){
	...
	url:"${base}/partybuilding/allparty/addLoad",
	title: "所有党员信息",
...
//编辑按钮方法，方法名固定，不可改变
	function editFun(code){
		if(code&&""!=$.trim(code)){
			var dialog=bootbox.dialog({
			url:"${base}/partybuilding/allparty/modifyLoad?code="+code,
			title: "所有党员信息",
			iframeStyle:"height:400px;width:730px;",
			style:'width:800px;height:520px;'
        });
		...
		var deleteFun=function(code){
			...
			bootbox.confirm('确定删除吗？',function(result){
				if(result){
					 $.ajax({
							type : "post",
							url : "${base}/partybuilding/allparty/delete?codes="+code,			
							dataType : 'json',
							async : false,
							success : function(data) {
	...
	$(function(){
		 $("#grid-table").jqGrid({
			 	...
			 	ondblClickRow:function(code){
				    bootbox.dialog({
						url:"${base}/partybuilding/allparty/infoSee?code="+code,
						title: "详细信息表",
		        ...
		        colModel:[
		            ...
		            {name:'seatGrid.gridTotalName',index:'gridCode',sortable:true, width:'24%',align:'center'},
...
<body>
	...
	<div class="col-xs-12 col-sm-10" id="body">

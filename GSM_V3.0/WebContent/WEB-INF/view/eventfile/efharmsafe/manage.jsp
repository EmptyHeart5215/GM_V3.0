	...
	<head>
		...
		var addFun=function(){
			var dialog=bootbox.dialog({
				 url:"${base}/eventfile/efharmsafe/addLoad",
				  title: "公共安全隐患及事件",
		...
		function editFun(code){
			if(code&&""!=$.trim(code)){
				var dialog=bootbox.dialog({
				 url:"${base}/eventfile/efharmsafe/modifyLoad?code="+code,
				  title: "公共安全隐患及事件",
		...
		var deleteFun=function(code){
			...
			bootbox.confirm('确定删除吗？',function(result){
				if(result){
					 $.ajax({
							type : "post",
							url : "${base}/eventfile/efharmsafe/delete?codes="+code,			
							dataType : 'json',
							async : false,
	...
	$(function(){
		 $("#grid-table").jqGrid({
			 	...
			 	ondblClickRow:function(rowid,iRow,iCol,e){
			    	  ...
			    	  bootbox.dialog({
							 url:"${base}/eventfile/efharmsafe/detailLoad?eventCode="+rowid+"&stepId=1&entryId=1",
							  title: "公共安全隐患及事件详细信息表",
							  iframeStyle:'height:1200px;',
							  style:'width:1000px;height:600px;',
							  showFunctionBtn:false,
							});
			      },
		        colNames:['序号', '所属辖区', '名称','事件发生时间','所属单位','隐患排查时间','负责人'],
		        colModel:[
								...
								<form id="search_form">
										...
										<tr>
											<td>
												<div class="search-group">
													...
													<input name=""codeHarmType" type="text" class="text col-xs-2"  >
										...
										<tr>
											<td>
														...
														<div class="search-group">
															<label class="col-xs-2  control-label text-right">地点或部位</label>
															<input name="place" type="text" class="text col-xs-2"  >
															<label class="col-xs-2  control-label text-right">负责人</label>
															<input name="unitName" type="text" class="text col-xs-2"  >
															<label class="col-xs-2  control-label text-right">事件发生时间</label>
															<div class="col-xs-2 input-group" style="padding-right: 0;padding-left: 0;">
													       <input  name="time" type="text" class="form-control date-picker tooltip-error required"  >

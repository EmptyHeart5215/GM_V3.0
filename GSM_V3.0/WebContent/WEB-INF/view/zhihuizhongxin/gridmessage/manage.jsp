...
<html lang="en">
	<head>
		...
		var addFun=function(){
			var dialog=bootbox.dialog({
				 url:"${base}/zhihuizhongxin/gridmessage/addLoad",
				  title: "网格员短信添加",
		...
		function editFun(code){
			if(code&&""!=$.trim(code)){
				var dialog=bootbox.dialog({
				 url:"${base}/zhihuizhongxin/gridmessage/modifyLoad?code="+code,
				  title: "网格员短信修改",
				  style:'width:800px;height:520px;'
				});
			}else{
		...
		var deleteFun=function(code){
			...
			bootbox.confirm('确定删除吗？',function(result){
				if(result){
					 $.ajax({
							type : "post",
							url : "${base}/zhihuizhongxin/gridmessage/delete?codes="+code,			
							dataType : 'json',
							async : false,
	...
	$(function(){
		 $("#grid-table").jqGrid({
			 	url : "${base }/zhihuizhongxin/gridmessage/queryManage",
		        ...
		        autowidth:true,//自动宽
		        defaultCaozuo:false,
		        colNames:[ '字典主键', '网格员姓名','手机号','网格名称','操作'],
		        colModel:[
					{name:'code',index:'code',sortable:false, hidden:true},
		            {name:'gridUserName',index:'gridUserName',sortable:true, width:'15%',align:'left'},
		            {name:'gridUserPhone',index:'gridUserPhone',sortable:true, width:'10%', align:"left"},

...
<html lang="en">
	<head>
		...
		var addFun=function(){
			var dialog=bootbox.dialog({
				 url:"${base}/zhihuizhongxin/hujiaochaxun/addLoad",
				  title: "答复记录添加",
		...
		function editFun(code){
			if(code&&""!=$.trim(code)){
				var dialog=bootbox.dialog({
				 url:"${base}/zhihuizhongxin/hujiaochaxun/modifyLoad?code="+code,
				  title: "答复记录修改",
				  style:'width:800px;height:520px;'
				});
			}else{
		...
		var deleteFun=function(code){
			...
			bootbox.confirm('确定删除吗？',function(result){
				if(result){
					 $.ajax({
							type : "post",
							url : "${base}/zhihuizhongxin/hujiaochaxun/delete?codes="+code,			
							dataType : 'json',
							async : false,
	...
	$(function(){
		 $("#grid-table").jqGrid({
			 	url : "${base }/zhihuizhongxin/hujiaochaxun/queryManage",
		        ...
		        autowidth:true,//自动宽
		        defaultCaozuo:false,
		        colNames:[ '字典主键', '登记人','事件标题','登记人住址','登记人身份证','操作'],
		        colModel:[
					{name:'code',index:'code',sortable:false, hidden:true},
		            {name:'callName',index:'callName',sortable:true, width:'15%',align:'left'},
		            {name:'eventTitle',index:'eventTitle',sortable:true, width:'10%', align:"left"},
		            {name:'address',index:'address',sortable:true, width:'20%', align:"left"},
		            {name:'appealCard',index:'appealCard',sortable:true, width:'15%',align:'left'},
										...
										<tr>
											<td>
												<div class="search-group">
													...
													<label class="col-xs-2 control-label text-right">事件标题</label>
													<input name="name" type="text" class="text col-xs-2"  >
													<label class="col-xs-2 control-label text-right">登记人证件号</label>
													<input name="name" type="text" class="text col-xs-2"  >

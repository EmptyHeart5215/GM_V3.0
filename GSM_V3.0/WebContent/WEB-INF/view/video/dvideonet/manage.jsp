...
<html lang="en">
	<head>
		...
		var addFun=function(){
			var dialog=bootbox.dialog({
				 url:"${base}/video/dvideonet/addLoad",
				  title: "新增视频网络",
		...
		function editFun(code){
			if(code&&""!=$.trim(code)){
				var dialog=bootbox.dialog({
				 url:"${base}/video/dvideonet/modifyLoad?code="+code,
				  title: "视频网络修改",
				  style:'width:800px;height:520px;'
				});
			}else{
		...
		var deleteFun=function(code){
			...
			bootbox.confirm('确定删除吗？',function(result){
				if(result){
					 $.ajax({
							type : "post",
							url : "${base}/video/dvideonet/delete?codes="+code,			
							dataType : 'json',
							async : false,
	...
	$(function(){
		 $("#grid-table").jqGrid({
			 	url : "${base }/video/dvideonet/queryManage",
		        ...
		        autowidth:true,//自动宽
		        defaultCaozuo:false,
		        colNames:[ '主键', '设备端口','连接IP','视频名称','网络端口','所属网格','用户名','密码','操作'],
		        colModel:[
					{name:'code',index:'code',sortable:false, hidden:true},
		            {name:'devicePort',index:'devicePort',sortable:true, width:'15%',align:'left'},
		            {name:'ip',index:'ip',sortable:true, width:'20%', align:"left"},
		            {name:'name',index:'name',sortable:true, width:'15%',align:'left'},
		            {name:'netPort',index:'netPort',sortable:true, width:'20%', align:"left"},
		            {name:'gridName',index:'gridName',sortable:true, width:'15%',align:'left'},
		            {name:'userName',index:'userName',sortable:true, width:'20%', align:"left"},
		            {name:'userPwd',index:'userPwd',sortable:true, width:'15%',align:'left'}

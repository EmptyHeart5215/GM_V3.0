...
<html lang="en">
	...
	function banLiFun(code,stepId,entryId){
			bootbox.dialog({
				 url:"${base}/event/eevent/approvalLoad?eventCode="+code+"&stepId=1&entryId=1",
				  title: "事件处理表",
	...
	$(function(){
		 $("#grid-table").jqGrid({
			 	url : "${base }/event/eevent/dubanManage",
		      defaultCaozuo:false,
		      multiselect:false,
		      ondblClickRow:function(rowid,iRow,iCol,e){
		    	  ...
		    	  var rowData = $('#grid-table').jqGrid("getRowData",code);
		    	   bootbox.dialog({
						 url:"${base}/event/eevent/detailLoad?eventCode="+rowData["code"]+"&stepId="+rowData['step.stepId']+"&entryId="+ rowData['step.entryId'],
						  title: "事件处理表",
						  iframeStyle:'height:1500px;',
						  style:'width:1000px;height:600px;',
		        ...
		        colNames:['序号','','', '受理编号', '所属辖区','事件主题','督办单位','督办时间','矛盾主题','事件类型','诉求人','督办理由'],
		        colModel:[
		         ...
		         /*    {name:'stepName',index:'stepName', width:80,fixed:true}, */
		            ...
		            {name:'petitionersName',index:'petitionersName', width:'5%'},
		            {name:'opinion',index:'opinion', width:'20%'}
		        ],
		        jsonReader:{

...
import com.dykj.GSM.mapper.event.impl.EContradictMapperImpl;
...
@Service
public class EContradictServiceImpl implements EContradictService {
	@Autowired
	private EOpinionMapperImpl eOpininonMapperImpl;
	@Autowired
	private EContradictMapperImpl eContradictMapperImpl;
	@Autowired
	...
	@Override
	public int insert(EContradict entity) throws Exception {
		// TODO Auto-generated method stub
		return 0;
	}

	@Override
	public int update(EContradict entity) throws Exception {
		// TODO Auto-generated method stub
		return 0;
	}
	...
	@Override
	public EContradict queryByKey(String primaryKey) throws Exception {
		// TODO Auto-generated method stub
		return null;
	}

	@Override
	public List<EContradict> queryList() throws Exception {
		// TODO Auto-generated method stub
		return null;
	}

	@Override
	public List<EContradict> queryListParam(Map param) throws Exception {
		// TODO Auto-generated method stub
		return null;
	}

	@Override
	public List<EContradict> queryTListParam(EContradict param) throws Exception {
		// TODO Auto-generated method stub
		return null;
	}

	@Override
	public Page<EContradict> queryByParamPage(int page, int size, String sidx, String sord, EContradict param)
			throws Exception {
		// TODO Auto-generated method stub
		return null;
	}
	...
	public int deleteBatch(List<String> list) throws Exception {
		return this.eContradictMapperImpl.deleteBatch(list);
	}
	...
	public int add(EContradict eevent, String userCode, String owner, EOpinion opinion) throws Exception {
		...
		// 将步骤的开始时间和办理时限存入map中
		putDatesToTransientVars(new Date(), transientVars, warnset.getDefaultBanLiDateContradict());
		long WfID = workflow.initialize(this.wfname, 受理ActionId, transientVars); // 初始化一条流程
		SimpleStep step = (SimpleStep) workflow.getCurrentSteps(WfID).get(0);
		Date createDate = step.getStartDate();
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.CHINA);
		eevent.setCreateDate(sdf.format(createDate));
	...
	public EContradict addDraft(EContradict eevent) throws Exception {
		if (eevent.getCreateDate() == null || eevent.getCreateDate().equals("")) {
		...
		if (eevent.getWfEntry() != null) {
			String eventNum = eevent.getCreateDate().replaceAll("-", "").replaceAll(" ", "").replaceAll(":", "");
			eevent.setEventNum(eventNum);
		}
		if (eevent.getCode() == null || "".equals(eevent.getCode().trim()) || eevent.getCode().equals("00000000")) {
			// 事件添加
			eContradictMapperImpl.insert(eevent);
		} else {
			eContradictMapperImpl.update(eevent);
		}
		return eevent;
	}
	...
	private void putDatesToTransientVars(Date startDate, Map<String, Object> transientVars, int duration)
		...
		if (mapList == null || mapList.isEmpty()) {
			throw new Exception("请先在节假日管理中设置！");
		}
	...
	public int modify(EContradict eevent) throws Exception {
		return eContradictMapperImpl.update(eevent);
	}
	...
	public EContradict queryByCode(String code) throws Exception {
		return eContradictMapperImpl.queryByKey(code);
	}
	...
	public Page<EContradict> queryDaiBanPage(String sidx, int rows, int page, String sord, Map<String, Object> args,
		...
		args.put("allSteps", stepMap);
		Page<EContradict> pageMap = eContradictMapperImpl.queryDaiBanPage(page, rows, sidx, sord, args);
		if (pageMap.getRecords() > 0) {
			dicValueInputService.inputDicValue(pageMap, EContradict.class, "codeEventType,codeEventFrom");
			if (!model.getRoleCode().equals(指挥中心权限Id)) {
				if (pageMap != null && pageMap.getRecords() > 0) {
					List<EContradict> listMaps = pageMap.getRows();
	...
	// 设置事件预警
	private void putAlertLight(List<EContradict> listMaps) throws Exception {
		WarningSet warnset = warningSetMapper.queryOne();
		if (warnset == null) {
			throw new Exception("请先进行预警提醒——提醒设置操作！");
		}
	...
	public Page<EContradict> queryYiBanPage(String sidx, int rows, int page, String sord, Map<String, Object> args,
		...
		Page<EContradict> pageMap = this.eContradictMapperImpl.queryYiBanPage(page, rows, sidx, sord, args);
		dicValueInputService.inputDicValue(pageMap, EContradict.class, "codeEventType,codeEventFrom");
		/*Map stepMap = (Map) getAllSteps().get("mapValue");
		for (EContradict e : pageMap.getRows()) {
	 ...
	 */
	@Override
	public Page<EContradict> queryCancelPage(String sidx, int rows, int page, String sord, Map<String, Object> args,
			String userCode) throws Exception {
		if (args == null) {
			args = new HashMap<String, Object>();
		}
		...
		Page<EContradict> pageMap = this.eContradictMapperImpl.queryCancelPage(page, rows, sidx, sord, args);
		dicValueInputService.inputDicValue(pageMap, EContradict.class, "codeEventType");
		/*Map stepMap = (Map) getAllSteps().get("mapValue");
		for (EContradict e : pageMap.getRows()) {
			String stepName = (String) stepMap.get(e.getStep().getStepId().toString());
			e.setStepName(stepName);
	 ...
	 */
	@Override
	public Page<EContradict> queryDuBanPage(String sidx, int rows, int page, String sord, Map<String, Object> args)
			throws Exception {
		if (args == null) {
			args = new HashMap<String, Object>();
		}
		...
		Page<EContradict> pageMap = this.eContradictMapperImpl.queryDuBanPage(page, rows, sidx, sord, args);
		dicValueInputService.inputDicValue(pageMap, EContradict.class, "codeEventType");
		/*Map stepMap = (Map) getAllSteps().get("mapValue");
		for (EContradict e : pageMap.getRows()) {
			String stepName = (String) stepMap.get(e.getStep().getStepId().toString());
	 ...
	 */
	@Override
	public Page<EContradict> queryWanJiePage(String sidx, int rows, int page, String sord, Map<String, Object> args)
			throws Exception {
		...
		Page<EContradict> pageMap = this.eContradictMapperImpl.queryWanJiePage(page, rows, sidx, sord, args);
		dicValueInputService.inputDicValue(pageMap, EContradict.class, "codeEventType");
		/*Map stepMap = (Map) getAllSteps().get("mapValue");
		for (EContradict e : pageMap.getRows()) {
			String stepName = (String) stepMap.get(e.getStep().getStepId().toString());
	...
	private Map<String, Object> queryApprovalDetail(Long wfid, String userCode, String roleCode, int stepID,
		...
		// 查询事件详细信息
		EContradict eevent = eContradictMapperImpl.queryByKey(code);
		if (eevent == null) {
			throw new Exception("查不到事件");
		}
		dicValueInputService.inputDicValueForOne(eevent, EContradict.class,
	...
	public int modifyApproval(String eventCode, long wfid, int stepID, int actionID, String owner, String userCode,
		...
		try {
			...
			if (actionID == 驳回延时ActionId) {
				...
				if (duration == null) {
					duration = warnset.getDefaultBanLiDateContradict();
				}
				...
				if (actionID != 立案派遣ActionId) {
					...
					putDatesToTransientVars(new Date(), map, warnset.getDefaultBanLiDateContradict());
				}
				...
				if (actionID == 申请延时ActionId) {
					...
					this.eContradictMapperImpl.updateEventCancel(eventCode);
				}
			}
			...
			if (currentSteps != null && !currentSteps.isEmpty()) {
				...
				if (actionID == 督办Id) {
					this.eContradictMapperImpl.udpateEventDuBan(eventCode);
				}
				EContradict contradict = eContradictMapperImpl.queryByKey(eventCode);
				String args = "{type:'矛盾',code:'" + eventCode + "',eventTitle:'" + contradict.getEventTitle()
						+ "',stepId:'" + currentSteps.get(0).getStepId() + "',entryId:'"
						+ currentSteps.get(0).getEntryId() + "'}";
				if (contradict != null) {
	 ...
	 */
	@Override
	public Page<EContradict> queryZongHePage(String sidx, int size, int pageNum, String sord, Map args)
			throws Exception {
		/*Integer[] actionIds = { 自办自结ActionId, 中心办结ActionId, 中心注销ActionId, 部门办结Id };
		String[] actionName = { "自办自结", "中心办结", "中心注销", "部门办结" };*/
		if (args == null) {
		...
		Page<EContradict> page = this.eContradictMapperImpl.queryZongHePage(pageNum, size, sidx, sord, args);
		dicValueInputService.inputDicValue(page, EContradict.class, "codeEventType,codeEventFrom");
		/*if (page != null && page.getRecords() > 0) {
			List<EContradict> listMaps = page.getRows();
			if (listMaps != null && !listMaps.isEmpty()) {
				this.putAlertLight(listMaps);
			}
		}*/
	 ...
	 */
	@Override
	public Page<EContradict> queryDraftPage(String sidx, int size, int pageNum, String sord, Map param)
			throws Exception {
		Page<EContradict> page = this.eContradictMapperImpl.queryDraftPage(pageNum, size, sidx, sord, param);
		dicValueInputService.inputDicValue(page, EContradict.class, "codeEventType,codeEventFrom");
		return page;
	}
	...
	*/
	static {
		...
		int i = prop.size();
		InputStream in = EContradictServiceImpl.class.getResourceAsStream("/workflowProperties.properties");
	...
	public Map<String, Object> queryAllStepsOfOneEvent(String entryId) throws FactoryException {
		...
		// 设置已办和在办的状态
		List<Map<String, Object>> list = this.eContradictMapperImpl.queryAllStepsOfOneEvent(entryId);
	...
	public List eventCharts(Map<String, Object> args) throws Exception {
		...
		args.put("type", type);
		String[] types = { "CODE_EVENT_TYPE", "GRID_CODE", "CODE_EVENT_GUIMO", "STEP_ID", "CODE_APPRAISE", "ACTION_ID" };
		...
		List<Map<String, Object>> list = this.eContradictMapperImpl.queryCharts(args);
		return list;
		/*if (list != null && list.size() > 0) {

			if (type.equals("CODE_EVENT_TYPE") || type.equals("CODE_EVENT_FROM") || type.equals("CODE_APPRAISE")) {
				Object[] keySet = null;
				List<Map<String, Object>> list0 = EventChartsDao.getAllSysValue();
				Map<String, Object> codeAppraiseMap = new HashMap<String, Object>();
				codeAppraiseMap.put("NAME_DIC", "CODE_APPRAISE");
				codeAppraiseMap.put("CODE", "未表态");
				codeAppraiseMap.put("NAME_C", "未表态");
				codeAppraiseMap.put("PARENT_CODE", "0000");
				list0.add(codeAppraiseMap);
				for (int i = 0; i < list.size() - 1; i++) {
					...
					for (Object k : keySet) {
						for (int beginIndex = list0.size() - 1; beginIndex >= 0; beginIndex--) {
							Map<String, Object> map0 = list0.get(beginIndex);
							if (map0.get("NAME_DIC") == null || map.get(k) == null) {
								continue;
							}
							// 找到第一条匹配的
							if (k.toString().matches(map0.get("NAME_DIC").toString())
									&& map.get(k).equals(map0.get("CODE"))) {
								if ("0000".equals(map0.get("PARENT_CODE"))) {
									map.put("GROUPTYPE", map0.get("NAME_C").toString());
								} else {
									String fieldName = k.toString();
									String dicValue = map0.get("NAME_C").toString();
									String name_c = EventChartsDao.getParentSysValue(beginIndex - 1, list0,
											map0.get("PARENT_CODE").toString(), fieldName, dicValue);
									map.put("GROUPTYPE", name_c);
								}
								// String
								// NAME_C=this.mateNames(map0.get("CODE").toString(),
								// list0,"NAME_C","CODE","PARENT_CODE");
							}
						}
					}
					Double count = Double.valueOf(map.get("COUNT").toString());
					map.put("PERCENTAGE",
							new BigDecimal(count * 100).divide(new BigDecimal(total), 2, BigDecimal.ROUND_UP)
									.doubleValue() + "%");
				}
			} else {
			String categories = "";
	...
	public List<Map<String, Object>> eventChartsByOrg(Map<String, Object> args) throws Exception {
		String[] types = { "DEPT", "CODE_EVENT_TYPE", "CODE_EVENT_GUIMO", "CODE_APPRAISE" };
		String type = args.get("type").toString();
		type = Utils.transformPropertyToColumn(type);
		args.put("type", type);
		if (!Arrays.asList(types).contains(type)) {
		...
		List<Map<String, Object>> list = eContradictMapperImpl.queryChartsByOrg(args);
	...
	public List<Map<String, Object>> mapqueryWanJiePage() {
		return eContradictMapperImpl.mapqueryWanJiePage();
	}
}

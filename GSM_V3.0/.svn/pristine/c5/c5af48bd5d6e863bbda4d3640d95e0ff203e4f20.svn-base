...
@RestController
@RequestMapping(value = "/yanpanfenxi/renchaxun")
public class RenChaXunAction extends BaseAction {
	private static Logger logger = Logger.getLogger(RenChaXunAction.class);
	@Autowired
	...
	public ModelAndView manageLoad() {
		return new ModelAndView("/yanpanfenxi/renchaxun/manage");
	}
	...
	public Object zongheManage(int page, int rows, String sidx, String sord, String args) {
		...
		try {
			...
			return bPersonService.queryZongHePage(sidx, rows, page, sord, m);
		} catch (Exception e) {
			e.printStackTrace();

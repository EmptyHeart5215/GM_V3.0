...
<html lang="en">
	...
	$(function(){
		 $("#grid-table").jqGrid({
			 	url : "${base }/base/perjingwai/queryManage",
		        defaultCaozuo:false,
		        ondblClickRow:function(rowid,iRow,iCol,e){
			    	  var code=$('#grid-table').jqGrid('getGridParam', "selrow");
			    	  var rowData = $('#grid-table').jqGrid("getRowData",code);
			    	  bootbox.dialog({
							 url:"${base}/base/person/infoSee?personCode="+rowData.code+"&personType=jingWai",
							  title: "人口信息采集表",
							  style:'width:1000px;height:600px;'
							});
			      },
		        colNames:[ '','', '所属辖区','楼栋','单元','房屋','证件号','姓名','性别','国籍','来华目的'],
		        colModel:[
					...
					{name:'buildName',sortable:false, width:'10%'},
					{name:'unitName',sortable:false, width:'5%'},
					{name:'houseNum',sortable:false, width:'5%'},
					{name:'cid',index:'code',sortable:false, width:'10%'},
					{name:'name',index:'code',sortable:false, width:'7%'},
					{name:'sex',index:'code',sortable:false, width:'5%'},
					{name:'dicValueMap.codeNationality',sortable:false, width:'15%'},
		            {name:'dicValueMap.codeComeChinaReason',sortable:false, width:'15%'}
								...
								<form id="search_form">
									<table style="width:100%;">
										...
										<tr>
											<td >
												<div class="search-group">
													<label class="col-xs-2 col-myxs-15 control-label text-right" >国籍</label>
													<input type="text"  name="codeNationality" class="col-xs-2 " />
													<label class="col-xs-2 col-myxs-15 control-label text-right" >来华目的</label>
													<input type="text"  name="codeComeChinaReason" class="col-xs-2 " />
												</div>
											</td>
										</tr>
									</table>

			...
			defaultCaozuo:false,
		 	ondblClickRow:function(rowid,iRow,iCol,e){
		         ...
		         var entryId = rowData['step.entryId'];
		 		  var show = "[";
		 		  show += "{\"NAME\":\""+Name+"\",\"POSITION\":\""+position+"\",\"IMG\":\"${base}/resource/js/map/img/2015062904.png\",\"HTML\":\"<div style='height:525px;width:800px;'><iframe width='100%' height='100%' marginheight='0' marginwidth='0' scrolling='no' frameborder='0' src='${base}/event/eevent/detailLoad?eventCode="+code+"&stepId="+stepId+"&entryId="+entryId+"'></iframe></div>\"},";
		 		 show = show.substring(0,show.length-1)+"]";
...
var mapSearchLoad = function (data){
	...
	if(rows.length>0){
		...
		for(var i  = 0;i<rows.length;i++){
			var row = rows[i];
	 		 var code = row.code;
	 		var stepId = row['step.stepId'];
	         var entryId = row['step.entryId'];
			show += "{\"NAME\":\""+row.eventTitle+"\",\"POSITION\":\""+row.position+"\",\"IMG\":\"${base}/resource/js/map/img/2015062904.png\",\"HTML\":\"<div style='height:525px;width:800px;'><iframe width='100%' height='100%' marginheight='0' marginwidth='0' scrolling='no' frameborder='0' src='${base}/event/eevent/detailLoad?eventCode="+code+"&stepId="+stepId+"&entryId="+entryId+"'></iframe></div>\"},";
		}
		show = show.substring(0,show.length-1)+"]";

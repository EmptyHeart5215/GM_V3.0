	...
	<head>
		...
		var addFun=function(){
			var dialog=bootbox.dialog({
				 url:"${base}/eventfile/efkeyplace/addLoad",
				  title: "治安重点地区",
				  iframeStyle:"height:750px;",
		...
		function editFun(code){
			if(code&&""!=$.trim(code)){
				var dialog=bootbox.dialog({
				 url:"${base}/eventfile/efkeyplace/modifyLoad?code="+code,
				  title: "治安重点地区",
		...
		var deleteFun=function(code){
			...
			bootbox.confirm('确定删除吗？',function(result){
				if(result){
					 $.ajax({
							type : "post",
							url : "${base}/eventfile/efkeyplace/delete?codes="+code,			
							dataType : 'json',
							async : false,
	...
	$(function(){
		 $("#grid-table").jqGrid({
			 	...
			 	ondblClickRow:function(rowid,iRow,iCol,e){
			    	  ...
			    	  bootbox.dialog({
							 url:"${base}/eventfile/efkeyplace/detailLoad?eventCode="+rowid+"&stepId=1&entryId=1",
							  title: "治安重点地区详细信息表",
		        ...
		        colNames:['序号', '所属辖区', '治安重点地区名称','主管或所属单位','单位负责人','管理负责人'],
		        colModel:[
										...
										<tr>
											<td>
														...
														<div class="search-group">
															...
															<input name="adminName" type="text" class="text col-xs-2" searchType="like" >
															<label class="col-xs-2  control-label text-right">存在的治安问题</label>
															<input name="securityPro" type="text" class="text col-xs-2" searchType="like" >

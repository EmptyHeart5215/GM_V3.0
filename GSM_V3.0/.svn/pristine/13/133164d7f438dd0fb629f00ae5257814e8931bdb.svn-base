...
@RestController
@RequestMapping(value = "/yanpanfenxi/bfeigongjingjiquery")
public class BFeiGongJingJiQueryAction extends BaseAction {
	private static Logger logger = Logger.getLogger(BFeiGongJingJiQueryAction.class);
	@Autowired
	...
	public ModelAndView manageLoad() {
		return new ModelAndView("/yanpanfenxi/bfeigongjingjiquery/manage");
	}
	...
	public Object zongheManage(int page, int rows, String sidx, String sord, String args) {
		...
		try {
			...
			return bFeiGongJingJiService.queryZongHePage(sidx, rows, page, sord, m);
		} catch (Exception e) {
			e.printStackTrace();

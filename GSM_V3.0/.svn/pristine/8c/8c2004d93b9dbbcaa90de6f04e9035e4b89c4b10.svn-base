...
//初始化区域图的对象
	...
	this.$bgDiv.append("<div class='GooFlow_work' style='overflow:hidden;width:"+(width)+"px;height:"+(height)+"px;"+(property.haveHead? "":"margin-top:3px")+"'></div>");
...
GooFlow.prototype={
	...
	initDraw:function(id,width,height){
		...
		if(GooFlow.prototype.useSVG!=""){
			...
			defs.appendChild(GooFlow.prototype.getSvgMarker("arrow_yiban","rgb(42, 137, 42)"));
		}
		else{
	...
	addNode:function(id,json){
		...
		if(json.type.indexOf(" round")<0){
			...
			if (json.type.indexOf(" yiban") > -1) this.$nodeDom[id].addClass("item_yiban");
			if (json.type.indexOf(" zaiban") > -1) this.$nodeDom[id].addClass("item_zaiban");
			if (json.type.indexOf(" weiban") > -1) this.$nodeDom[id].addClass("item_weiban");
		}
		else{
    ...
    //增加图例
	addTuli: function (id) {
	    this.$nodeDom[id] = $("<div class='GooFlow_item' id='" + id + "' style='top:10px;left:10px'><table cellspacing='0'><tr><td class='item_yiban'>&nbsp;&nbsp;&nbsp;&nbsp;</td><td>&nbsp;已办</td></tr><tr><td class='item_zaiban'></td><td>&nbsp;在办</td></tr><tr><td class='item_weiban'></td><td>&nbsp;未办</td></tr></div>");
	    this.$workArea.append(this.$nodeDom[id]);
	},
...
///////////以下为有关画线的方法
	...
	drawLine1:function(id,sp,ep,mark,dash,status){
		var line;
		if(GooFlow.prototype.useSVG!=""){
			line=document.createElementNS("http://www.w3.org/2000/svg","g");
			var hi=document.createElementNS("http://www.w3.org/2000/svg","path");
			...
			if(typeof status!="undefined"&&status=="yiban"){
				path.setAttribute("stroke","rgb(42,137,42)");
				path.setAttribute("marker-end","url(#arrow2)");
			}
			line.appendChild(hi);
			line.appendChild(path);
			...
			if(typeof status!="undefined"&&status=="yiban"){
				line.strokeColor="rgb(42,137,42)";
			}
	...
	//画一条只有两个中点的折线
	drawPoly:function(id,sp,m1,m2,ep,mark,dash){
		var poly,strPath;
		if(GooFlow.prototype.useSVG!=""){
	...
	addLine:function(id,json){
		...
		if(this.$lineData[id].type=="sl")
			this.$lineDom[id]=GooFlow.prototype.drawLine(id,res.start,res.end,json.mark,json.dash);
		else
			this.$lineDom[id]=GooFlow.prototype.drawPoly(id,res.start,res.m1,res.m2,res.end,json.mark,json.dash);
	...
	resetLines:function(id,node){
		for(var i in this.$lineData){
		  ...
		  if(this.$lineData[i].type=="sl"){
		  	this.$lineDom[i]=GooFlow.prototype.drawLine(i,res.start,res.end,this.$lineData[i].marked,this.$lineData[i].dash);
		  }
		  else{
			this.$lineDom[i]=GooFlow.prototype.drawPoly(i,res.start,res.m1,res.m2,res.end,this.$lineData[i].marked,this.$lineData[i].dash);
		  }
	...
	setLineType:function(id,newType){
		...
		if(newType!="sl"){
		  ...
		  this.setLineM(id,this.getMValue(this.$nodeData[from],this.$nodeData[to],newType));
		}
		//如果是变回直线
		else{
	...
	setLineM:function(id,M,noStack){
		...
		this.$draw.removeChild(this.$lineDom[id]);
		this.$lineDom[id]=GooFlow.prototype.drawPoly(id,ps.start,ps.m1,ps.m2,ps.end,this.$lineData[id].marked||this.$focus==id,this.$lineData[id].dash);
		this.$draw.appendChild(this.$lineDom[id]);
		if(GooFlow.prototype.useSVG==""){
			this.$lineDom[id].childNodes[1].innerHTML=this.$lineData[id].name;

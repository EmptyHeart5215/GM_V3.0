...
<html lang="en">
	...
	$(function(){
		...
		minViewMode:'months'
		 ...
		 $("#grid-table").jqGrid({
			 	url : "${base}/eva/appraisestreet/queryAppraisedStreet?date=${data.nowDate}&gridCode=${data.gridCode}",
		        defaultCaozuo:false,
		        multiselect:false,
		        ondblClickRow:function(rowid,iRow,iCol,e){
			    	  var code=$('#grid-table').jqGrid('getGridParam', "selrow");
			    	  bootbox.dialog({
							 url:"${base}/eva/appraisestreet/queryAppraisedStreetDetail?code="+code,
							  title: "详细列表",
		        ...
		        colNames:[ '字典主键', '考核对象','年','月','操作时间','矛盾未按时办结扣分','矛盾返工扣分','矛盾满意度扣分','矛盾得分','事件未按时办结扣分','事件返工扣分','事件满意度扣分','事件得分','社区平均分','总分'],
		        colModel:[
		            ...
		            {name:'totalScore',index:'eventScore', width:'10%'},
		        ],
		        sortable:true,
		 ...
		 $("#search").click(function(){
			 ...
			 bootbox.confirm('考核数据将被覆盖，确定要进行操作？',function(result){
					if(result){
						 $.ajax({
								type : "post",
								url : "${base}/eva/appraisestreet/appraiseStreet?date="+date+"&gridCode="+gridCode,			
								dataType : 'json',
								async : false,
								success : function(data) {
									...
									if(data.status){
										...
										$('#grid-table').jqGrid('setGridParam', {url:"${base}/eva/appraisestreet/queryAppraisedStreet?date="+date+"&gridCode="+gridCode}).trigger('reloadGrid');
									}
								}
							}); 
					}

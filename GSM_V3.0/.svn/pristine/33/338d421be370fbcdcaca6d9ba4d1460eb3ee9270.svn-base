...
<html lang="en">
		...
		function editFun(code){
			if(code&&""!=$.trim(code)){
				var dialog=bootbox.dialog({
				 url:"${base}/event/eevent/editDraftLoad?code="+code,
				  title: "事件处理表",
	...
	$(function(){
		 $("#grid-table").jqGrid({
			 	url : "${base }/event/eevent/draftManage",
		      defaultCaozuo:false,
		      multiselect:true,
		      ondblClickRow:function(rowid,iRow,iCol,e){
		        ...
		        colNames:['序号',  '所属辖区','诉求人','事件主题','事件类型','上报来源','创建时间','操作'],
		        colModel:[
					{name:'code',index:'code',sortable:false, hidden:true},
		            {name:'gridTotalName',index:'gridTotalName', width:'30%'},
		            {name:'petitionersName',index:'petitionersName', width:'8%'},

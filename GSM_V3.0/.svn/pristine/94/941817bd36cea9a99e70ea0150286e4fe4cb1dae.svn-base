...
<html lang="en">
	<head>
		...
		function editFun(code){
			if(code&&""!=$.trim(code)){
				var dialog=bootbox.dialog({
				 url:"${base}/video/dvideo/modifyLoad?code="+code,
				  title: "视频修改",
				  style:'width:800px;height:520px;'
				});
			}else{
		...
		var deleteFun=function(code){
			...
			bootbox.confirm('确定删除吗？',function(result){
				if(result){
					 $.ajax({
							type : "post",
							url : "${base}/video/dvideo/delete?codes="+code,
							dataType : 'json',
							async : false,
	...
	$(function(){
		 $("#grid-table").jqGrid({
			 	url : "${base }/video/dvideo/queryManage",
		        ...
		        autowidth:true,//自动宽
		        defaultCaozuo:false,
		        colNames:[ '主键', '视频名称','视频地址','所属辖区','编码','类型','通道','地图坐标','操作'],
		        colModel:[
					{name:'code',index:'code',sortable:false, hidden:true},
		            {name:'videoName',index:'videoName',sortable:true, width:'15%',align:'left'},
		            {name:'videoAddress',index:'videoAddress',sortable:true, width:'20%', align:"left"},
		            {name:'gridName',index:'gridName',sortable:true, width:'15%',align:'left'},
		            {name:'codeVideonet',index:'codeVideonet',sortable:true, width:'20%', align:"left"},
		            {name:'typeVideoname',index:'typeVideoname',sortable:true, width:'15%',align:'left'},
		            {name:'codeChannel',index:'codeChannel',sortable:true, width:'20%', align:"left"},
		            {name:'position',index:'position',sortable:true, width:'15%',align:'left'}
										...
										<tr>
											<td>
												<div class="search-group">
													...
													<label class="col-xs-2 control-label text-right">视频名称</label>
													<input name="name" type="text" class="text col-xs-2"  >
													<label class="col-xs-2 control-label text-right">视频通道</label>
													<input name="name" type="text" class="text col-xs-2"  >

	...
	<head>
		...
		var addFun=function(){
			var dialog=bootbox.dialog({
				 url:"${base}/base/bfeigongjingji/addLoad",
				  title: "非公经济组织添加",
		...
		function editFun(code){
			if(code&&""!=$.trim(code)){
				var dialog=bootbox.dialog({
				 url:"${base}/base/bfeigongjingji/modifyLoad?code="+code,
				  title: "非公经济组织修改",
				  iframeStyle:"height:500px;",
				  style:'width:850px;height:520px;'
				});
		...
		var deleteFun=function(code){
			 ...
			 bootbox.confirm('确定删除吗？',function(result){
				if(result){
					 $.ajax({
							type : "post",
							url : "${base}/base/bfeigongjingji/delete",
							 data:{"codes":code},
							dataType : 'json',
							async : false,
	...
	$(function(){
		 $("#grid-table").jqGrid({
		        ...
		        ondblClickRow:function(rowid,iRow,iCol,e){
				    bootbox.dialog({
						url:"${base}/base/bfeigongjingji/infoSee?code="+rowid,
						title: "非公有经济组织信息采集表",
		        ...
		        colNames:[ '字典主键', '非公有制经济组织名称','所属辖区','非公有制经济组织地址','非公有制经济组织负责人',
		                   '网格工作联系人','网格工作联系人电话','操作'
		                   ],
		        colModel:[
					{name:'code',index:'code',sortable:false, hidden:true},
		            {name:'name',index:'name',sortable:true, width:'15%',align:'center'},
		            {name:'seatGrid.gridTotalName',index:'gridCode',sortable:true, width:'20%', align:"center"},
		            {name:'address',index:'address',sortable:true, width:'20%', align:"center"},
		            {name:'manager',index:'manager',sortable:true, width:'20%', align:"center"},

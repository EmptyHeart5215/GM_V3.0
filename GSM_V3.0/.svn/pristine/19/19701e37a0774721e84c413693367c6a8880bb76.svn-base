...
<%@page import="com.dykj.GSM.service.event.impl.EEventServiceImpl" %>
...
$(function(){
	...
	$("#form").validate({
		submitHandler: function (form) {
			var data = $("#form").serialize();
			 $.ajax({
				type : "post",
				url : "${base}/event/eevent/approval?t="+Math.random(),
				data : data,
				dataType : 'json',
				async : false,
  ...
  var demo = $.createGooFlow($("#demo"), property);
   var jsonPara={
			type:"post",
			url:"${base}/event/eevent/queryFlowSteps",
			//data:{entryId:241}
...
EEvent event=(EEvent)vdata.get("event");
		...
		<div class="tabbable tabs-left">
			...
			<!-- 基本信息 --> 
				...
				<div class='ui-pg-div ui-inline-edit'title='打印' style='cursor:pointer;' onclick="print('print')">
					...
					<div id="print">
					<h2><center>事件受理表</center></h2>
					<h5>事件编号：${vdata.event.eventNum }</h5>
						<table class="print-table" >
							<tr>
								<td class="col-xs-2">所属网格</td><td class="print-normal col-xs-4">${vdata.event.gridTotalName}</td>
								<td class="col-xs-2">事件类型</td><td class="print-normal col-xs-4">${vdata.event.dicValueMap.codeEventType}</td>
							</tr>
							<tr>
								<td class="col-xs-2">事件主题</td><td colspan="3" class="print-normal col-xs-10">${vdata.event.eventTitle}</td>
							</tr>
							<tr>
							...
							<tr>
							<td class="col-xs-2">诉求人姓名</td><td class="print-normal col-xs-4">${vdata.event.petitionersName }</td>
							<td class="col-xs-2">诉求人性别</td><td class="print-normal col-xs-4">${vdata.event.petitionersSex}</td>
							</tr>
							<tr>
							<td class="col-xs-2">诉求人电话</td><td class="print-normal col-xs-4">${vdata.event.petitionersPhone}</td>
							<td class="col-xs-2">共同诉求人</td><td class="print-normal col-xs-4">${vdata.event.petitionersPersons}</td></tr>
							<tr><td class="col-xs-2">诉求人地址</td><td colspan="3" class="print-normal col-xs-10">${vdata.event.petitionersAddress}</td></tr>
							
							<tr>
								<td class="col-xs-2">事件详情</td><td colspan="3" class="print-normal col-xs-10">${vdata.event.description}</td>
							</tr>
							<tr>
				...
				<div id="workflowDiv" class="tab-pane">
					<div id="demo" class="GooFlow" style="width: 698px; height: 620px;">
					</div>
				</div>
				<c:if test="${vdata.approval}">

...
<html lang="en">
	<head>
		...
		function editFun(code){
			if(code&&""!=$.trim(code)){
				var dialog=bootbox.dialog({
				 url:"${base}/sys/rnotice/modifyLoad?code="+code,
				  title: "公告修改",
				  iframeStyle:"height:500px;",
				  style:'width:800px;height:520px;'
				});
		...
		var deleteFun=function(code){
			...
			bootbox.confirm('确定删除吗？',function(result){
				if(result){
					 $.ajax({
							type : "post",
							url : "${base}/sys/rnotice/delete?codes="+code,			
							dataType : 'json',
							async : false,
	...
	$(function(){
		 $("#grid-table").jqGrid({
		        ...
		        colNames:[ '字典主键', '标题','内容','发布时间','发布人'],
		        colModel:[
					{name:'code',index:'code',sortable:false, hidden:true},
		            {name:'title',index:'title',sortable:false, width:'10%'},
		            {name:'content',index:'content',sortable:false, width:'25%'},
		            {name:'createDate',index:'createDate',sortable:false, width:'10%'},
		            {name:'sysUser.realName',sortable:false, width:'10%'}
		        ],
		        sortable:true,
		        altRows:true,//班玛线
		        sortname:'createDate',
										...
										<tr>
											<td>
												<div class="search-group">
													...
													<input type="text" name="title" class="text col-xs-2" />
													<label class="col-xs-2 control-label no-padding-right text-right" >注册时间</label>
